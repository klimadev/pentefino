...
async def scan_dns(domain: str) -> dict:
    ...
    mx_provider = ""
    mx_lines = [ln.strip() for ln in mx.split("\n") if ln.strip()]
    # strip priority, get domain
    mx_domains = [re.sub(r"^\d+\s+", "", ln).rstrip(".") for ln in mx_lines[:3]]
    if mx_domains:
    ...
    spf = any("v=spf1" in ln for ln in txt_raw.split("\n") if ln.strip())
    dmarc = any("v=DMARC1" in ln for ln in dmarc_raw.split("\n") if ln.strip())
    return dict(
        a=a.split("\n")[0] if a else "",
        aaaa=aaaa.split("\n")[0] if aaaa else "",
        cname=cname.split("\n")[0] if cname else "",
        ...
        ns_list=[ln.strip().rstrip(".") for ln in ns.split("\n") if ln.strip()],
        txt_list=[ln.strip().strip('"') for ln in txt.split("\n") if ln.strip()],
        spf=spf,
        dmarc=dmarc,
    )
...
async def scan_stack(domain: str) -> dict:
    ...
    return dict(status_code=status_code, title=title, webserver=webserver, cpe=cpe, nextjs=nextjs, whatweb=whatweb)
...
async def scan_perf(domain: str, strategy: str, tmpdir: Path) -> dict:
    ...
    await sh(
        f"npx --yes lighthouse https://{domain} --output json --output-path {out} "
        f'--chrome-flags="--no-sandbox --headless" --quiet {preset}',
        120,
    )
    if not out.exists():
    ...
    return dict(
        scores=dict(
            ...
            best_practices=gs("best-practices"),
            seo=gs("seo"),
        ),
        cwv=dict(
            lcp_ms=ga("largest-contentful-paint"),
            cls=ga("cumulative-layout-shift"),
            tbt_ms=ga("total-blocking-time"),
            fcp_ms=ga("first-contentful-paint"),
...
def _print_summary(r: dict, log):
    ...
    if not dns.get("dmarc"):
        issue(f"{R}❌{N}", f"{BO}DMARC não configurado{N} — vulnerável a spoofing")
    if dns.get("spf"):
        issue(f"{G}✅{N}", f"{BO}SPF configurado{N}")
    log(f"\n  {BO}🥞 Stack{N}")
    cpe = stk.get("cpe", [])
    log(f"    {cpe[0] if cpe else 'Framework: Não identificado'}")
    ...
    for mode in ("mobile", "desktop"):
        ...
        if p and p < 90:
            issue(f"{Y}⚠️{N}", f"{BO}{mode.title()}: Performance {p}%{N} (LCP {lcp / 1000:.1f}s)")
    vc = r.get("visual_critique")
    if vc and vc.get("design_quality"):
        dq = vc["design_quality"]
        if dq < 5:
            issue(f"{R}🎨{N}", f"{BO}Design visual: {dq}/10{N} — revisar layout/UI")
        log(f"\n  {BO}🎨 Design Visual{N}")
        log(f"    Qualidade: {dq}/10")
        if vc.get("key_issues"):
            for iss in vc["key_issues"][:3]:
                log(f"    • {iss}")
    log(f"\n  {BO}🔍 Recomendações:{N}")
    if not dns.get("dmarc"):
        log(f"    • {R}CRÍTICO{N}: Configure DMARC")
    lcp_m = perf.get("mobile", {}).get("cwv", {}).get("lcp_ms", 0)
    if lcp_m > 2500:
        log(f"    • {Y}LENTO{N}: LCP mobile alto ({lcp_m:.0f}ms). Otimizar imagens, JS, caching")
...
async def scan(target: str, prompt: str | None = None, opts: dict | None = None) -> dict:
    ...
    labels = dict(
        registration="Registro (RDAP + WHOIS)",
        dns="DNS",
        subdomains="Subdomínios",
        stack="Stack",
        html="HTML",
        perf_mobile="⚡ Performance (Mobile)",
    ...
    if d.get("cname"):
        log(f"  {BO}CNAME:{N}{d['cname']}")
    if d.get("soa"):
        log(f"  {BO}SOA:{N}  {d['soa']}")
    log(f"  {BO}MX:{N}")
    for mx in d.get("mx_list", []):
        log(f"    {mx}")
    log(f"  {BO}NS:{N}")
    for ns in d.get("ns_list", []):
        log(f"    {ns}")
    log(f"  {BO}TXT:{N}")
    for tx in d.get("txt_list", [])[:10]:
        log(f"    {tx}")
    ...
    if vis:
        ...
        if vis and vis.get("design_quality"):
            ...
            if vis.get("layout"):
                log(f"  {BO}Layout:{N} {vis['layout']}")
            if vis.get("colors"):
                log(f"  {BO}Cores:{N} {vis['colors']}")
            if vis.get("typography"):
                log(f"  {BO}Tipografia:{N} {vis['typography']}")
            if vis.get("consistency"):
                log(f"  {BO}Consistência:{N} {vis['consistency']}")
            log(f"  {BO}Mobile-friendly:{N} {'✅ Sim' if vis.get('mobile_friendly') else '❌ Não'}")
            if vis.get("key_issues"):
                log(f"  {BO}Problemas:{N}")
                for iss in vis["key_issues"]:
                    log(f"    • {iss}")
            if vis.get("recommendations"):
                log(f"  {BO}Recomendações:{N}")
                for rec in vis["recommendations"]:
                    log(f"    • {rec}")
        else:
...
def _build_json_custom(r: dict, domain: str, out_json: Path, log):
    ...
    def _s(s):
        if not s:
            return []
        items = s if isinstance(s, list) else s.split("\n")
        return [_n(x) for x in items if x]

    now_ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    scan = dict(domain=domain, timestamp=now_ts, tool="pentefino.py")
    registration = dict(
        registrar=_n(reg.get("registrar")),
        registrant_name=_n(reg.get("registrant_name")),
        registrant_org=_n(reg.get("registrant_org")),
        registrant_email=_n(reg.get("registrant_email")),
        registrant_country=_n(reg.get("registrant_country")),
        creation_date=_n(reg.get("creation")),
        expiration_date=_n(reg.get("expiration")),
    )
    dns_out = dict(
        a=_s(dns.get("a", "")),
        aaaa=_s(dns.get("aaaa", "")),
        cname=_n(dns.get("cname", "")),
        mx=_s(dns.get("mx_list", [])),
        ns=_s(dns.get("ns_list", [])),
        txt=_s(dns.get("txt_list", [])),
        soa=_n(dns.get("soa", "")),
        spf=_n(dns.get("spf")),
        dmarc=_n(dns.get("dmarc")),

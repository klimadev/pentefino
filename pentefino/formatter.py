...
def print_lighthouse(log, label: str, data: dict):
    ...
    p = sc.get("performance", 0)
    a = sc.get("accessibility", 0)
    bp = sc.get("best_practices", 0)
    seo = sc.get("seo", 0)
    lcp = cw.get("lcp_ms", 0)
    cls = cw.get("cls", 0)
    tbt = cw.get("tbt_ms", 0)
    ...
    def e(v):
        return "✅" if v >= 90 else "⚠️" if v >= 50 else "❌"
    ...
    for k, v in r.items():
        ...
        if isinstance(v, dict):
            ...
            for sk, sv in v.items():
                short = json.dumps(sv, ensure_ascii=False)[:200]
                log(f"    {sk}: {sv}" if not isinstance(sv, (dict, list)) else f"    {sk}: {short}")

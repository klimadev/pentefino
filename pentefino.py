...
from pentefino.platforms import resolve, list_platforms
from pentefino.formatter import R, G, Y, B, N, BO


async def scan(target: str, visual: bool = False, prompt: str | None = None,
               platform_hint: str | None = None, batch: bool = False) -> dict | None:
...
async def scan_all(targets: list[str], visual: bool = False, prompt: str | None = None,
    ...
    results = await asyncio.gather(*[
        scan(t, visual=visual, prompt=prompt, platform_hint=platform_hint, batch=True)
        for t in targets
    ])
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='pentefino.py',
        description='pentefino — OSINT + recon + perf + visual / Instagram critique')
    parser.add_argument('targets', nargs='*', help='target(s): domain, @username, or URL')
    parser.add_argument('--platform', '-p', metavar='NAME', default=None,
        help=f'specific platform: {", ".join(p["name"] for p in list_platforms())} (default: auto-detect)')
    parser.add_argument('--prompt', '-P', metavar='TEXT', default=None,
        help='custom AI prompt for visual/instagram analysis')
    parser.add_argument('--visual-critique', '-v', action='store_true',
        help='enable visual design critique (site platform only)')
